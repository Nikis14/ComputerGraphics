...
namespace Task
{
    public partial class Form1 : Form
    {
        ...
        private bool get_intersect(PointF a, PointF b, PointF c, PointF d, ref PointF res)
        {
            ...
            float x = (float)(a.X + (b.X - a.X) * t1);
            float y = (float)(a.Y + (b.Y - a.Y) * t1);
            res = new PointF(x, y);
            return true;
        }
        ...
        private void choose_method()
        {
            ...
            switch (comboBox1.SelectedItem.ToString())
            {
                ...
                case "Принадлежит ли точка многоугольнику":
                    ...
                    int cnt = 0;
                    bool f = false;
                    list = list.Distinct().ToList<PointF>();
                    for (int i = 0; i < list.Count(); ++i)
                    {
                        ...
                        if (res.X == -1)
                            continue;
                        bool betw = distance(res, start) <= distance(start, check);
                        if (res.X == list[ind].X && res.Y == list[ind].Y && betw)
                        {
                            //f = true;
                            int cnt2 = 0;
                            if (list[i].Y < list[ind].Y)
                                cnt2--;
                            else if (list[i].Y == list[ind].Y)
                                continue;
                            else
                                cnt2++;


                            if (list[(ind + 1) % list.Count()].Y < list[ind].Y)
                                cnt2--;
                            else if (list[(ind+1) % list.Count()].Y > list[ind].Y)
                                cnt2++;
                        ...
                        bool betw_lists = PointF_between(res, list[i], list[ind]);
                        if (is_inter && betw_lists && betw)
                            cnt++;
                    }
